...
```php
add_action( 'surecart/order_created', function( $order, $data ) {
    // Access the checkout from the order (requires expansion)
    ...
    foreach ( $line_items as $item ) {
        if ( ! empty( $item->price->product->shipping_enabled ) ) {
            $has_physical = true;
            break;
        }
    }
...
```php
add_action( 'surecart/invoice_created', function( $invoice, $data ) {
    // Access the checkout from the invoice (requires expansion)
