...
<Warning>
  ...
  `appearance`, and `locale` apply in both cases.
</Warning>
...
Restrict which payment methods appear when a particular product is in the cart. For example, to offer only card and Link for one product:
...
<Note>
  `paymentMethodTypes` is an allowlist, not a blocklist. It replaces Stripe's
  automatic payment methods, so you must list every method you want to keep and
  each one must be enabled in your Stripe dashboard. Any method you omit,
  including Klarna and other buy now, pay later options, will not appear.
</Note>
