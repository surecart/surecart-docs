...
Control whether new users see the password change nag after their account is created with a random, temporary password. Use the `get_user_metadata` filter to intercept the `default_password_nag` user meta check.
